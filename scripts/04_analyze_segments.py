...
def generate_bi_data(spark):
    ...
    df = df.withColumnRenamed("prediction", "customer segment")

    # 1. Create Aggregated Summary for High-Level KPI cards
    summary_df = df.groupBy("customer segment").agg(
    ...
    labeled_summary = summary_df.withColumn(
        "Segment_Label",
        when((col("Avg_Recency") < 30) & (col("Avg_Frequency") >= 5) & (col("Avg_Monetary") >= 500), "Low R, High F, High M - Champion - Reward them, ask for referrals, give early access to new products.")
        .when((col("Avg_Recency") >= 90) & (col("Avg_Frequency") < 2) & (col("Avg_Monetary") < 100), "High R, Low F, Low M - At-Risk/Churned - Use win-back campaigns, surveys, or heavy discounts to re-engage.")
        .when((col("Avg_Recency") < 30) & (col("Avg_Frequency") < 2) & (col("Avg_Monetary") < 100), "Low R, Low F, Low M - New Customer - Nurture them with welcome content, explain the brand value.")
        .when((col("Avg_Recency") >= 30) & (col("Avg_Frequency") >= 5) & (col("Avg_Monetary") >= 500), "High R, High F, High M - Loyal/High Value - Keep them satisfied; personalize their experience so they stay.")
        .otherwise("Standard")
    )
